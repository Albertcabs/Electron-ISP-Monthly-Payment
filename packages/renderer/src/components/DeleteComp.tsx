...
const DeleteComp = ({ resHead }: Props) => {
   ...
   const deleteHandler = async () => {
      let isDone = false;
      try {
         isDone = await window.api.deletePerson([data.key[0], data.key[1]]);
      } catch (err: any) {
   ...
   return (
      <div className='absolute top-20 z-50 min-h-max  w-full '>
         <div className='mx-auto min-h-max w-[350px] flex-col rounded-xl border-2 border-green-600 px-6 py-4 bg-slate-900 '>
            <svg
               aria-hidden='true'
